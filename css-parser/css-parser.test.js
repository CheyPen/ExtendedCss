(function () {
    ...
    var _exports = exports,
        ExtendedCssParser = _exports.ExtendedCssParser;
    ...
    QUnit.test('Simple CSS', function (assert) {
      var selector = 'body';
      var cssText = "".concat(selector, " { display:none; }");
    ...
    QUnit.test('Test Sizzle tokenize cache', function (assert) {
      var selector = 'body';
      var cssText = "".concat(selector, " { display:none; }");
      ExtendedCssParser.parseCss(cssText); // Check the tokens cache only now

      var tokens = Sizzle.tokenize(selector, false, {
        cacheOnly: true
      });
      assert.ok(tokens);
    ...
    QUnit.test('Parse stylesheet with extended selectors', function (assert) {
      var cssText = ':contains(/[\\w]{9,}/){display:none!important;visibility:hidden!important}\
